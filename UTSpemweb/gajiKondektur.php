        ...
        <table border="1" >
          ...
          <?php while($data = mysqli_fetch_array($result)): ?>
                    <tr>
                        ...
                        if(isset($_GET['bulan'])){
                            $bulan = $_GET['bulan'];
                            $query = "SELECT SUM(jumlah_km) AS total_km FROM trans_upn WHERE id_kondektur = $data[id_kondektur] AND monthname(tanggal) = '$bulan' GROUP BY id_kondektur";
                        }else{
                        ...
                        $dataKondektur = mysqli_fetch_assoc($result_KM);
                        if($dataKondektur === NULL) {
                            $total_km = 0;
                        } else {
                            $total_km = $dataKondektur['total_km'];
                        }
                        $kondektur_salarykm = 1500;
                        $kondektur_salary = $total_km * $kondektur_salarykm;
                        ?>
